...
begin
 case CommandType of
    tcInsert:
    begin;
     PKDBDictLabelEdit3.SetValue('1996842');      // Московская обл
...
begin      // ПОСЛЕ ВЫБОРА РАЙОНА ГОРОДА
inherited;
...
procedure TADDRESS.CitySelect(Sender: TObject);       // ПОСЛЕ ВЫБОРА ГОРОДА
begin
  if PKDBBLabelComboBox3.Text='' then
   begin  // ОЧИЩАЕМ СВЯЗАННЫЕ ПОЛЯ
    PKDBBLabelComboBox3.selectid:='';
...
begin   // ПОСЛЕ СМЕНЫ РЕГИОНА
   ...
   PKDBBLabelComboBox3.prepearedate;
   PKDBBLabelComboBox3.getDBdata;
   // обновляем районы/поселки
   PKDBBLabelComboBox2.selectid:='';
   PKDBBLabelComboBox2.constraint:= ' townid='+PKDBDictLabelEdit3.DictionaryValue;
   PKDBBLabelComboBox2.prepearedate;
   PKDBBLabelComboBox2.getDBdata;
   // меняем список улиц города (тк есть подчиненные непосредственно Мосве и тд)
   PKDBBLabelComboBox1.selectid:='';
