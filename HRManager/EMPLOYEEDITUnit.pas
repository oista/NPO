...
type
  TEMPLOYEEDIT = class(TEditDialog)
    ...
    ADQuery3: TADQuery;
    ADQuery3STRUCTUREID: TFMTBCDField;
    ADQuery3DESCRIPTION: TStringField;
...
var
  ...
  DefAccType: string = '3075940';     // доступ по умолчанию - есть
  DefSource:  string = '3081505';     // источник компл по умолчанию - вольный найм
  DefSourceT: string = '3075942';     // тип источника - внешний
...
procedure TEMPLOYEEDIT.FormShow(Sender: TObject);
begin
  inherited;
  // передаем значение гражданина компоненту
  PKDBDictEdit1.SetValue(PKDBTable1.Fields.ParentKey.FieldVal);
end;
...
begin
    ...
    tcInsert:
      begin
       ...
       begin
        ...
        PKDBBLabelComboBox1.SetValue( DefSource );
        // тип источника комплектования = внешний
        PKDBBLabelComboBox3.SetValue( DefSourceT );
       end;
      end;
    tcUpdate:
      Begin
  ...
  if CommandType <> tcInsert then
   begin
    ...
    // конвертируем испытательный срок в дни
   //  if (PKDBBaseEdit2.Text<>'') and (PKDBLabelEdit4.Text<>'') then
     //  PKDBBaseEdit1.Text:= floattostr(strtodate
       //    (PKDBBaseEdit2.Text)-strtodate(PKDBLabelEdit4.Text));
  {  if (PKDBDictEdit3.<>'') and (PKDBLabelEdit4.Text<>'') then
       PKDBBaseEdit1.Text:= floattostr(strtodate
           (PKDBDictEdit3.DictionaryValue)-strtodate(PKDBLabelEdit4.Text));    }
   end;
 ...
 { if ( CommandType=tcInsert )  then
    begin
         // для сотр работавших по договору подтягиваем старую должность(при включении в штат)
      ADQuery3.Params[0].Value := PKDBTable1.Fields.ParentKey.FieldVal;

      ADQuery3.open;
      if (ADQuery3STRUCTUREID.asstring<>'') then
     // проверяем есть ли в штате данная единица или это внештатный сотрудник
     begin
        if (ADQuery3DESCRIPTION.asstring='ВНЕШТАТНЫЙ СОСТАВ') then
          showmessage('Сотрудник работал на внештатной должности.'
                    +' Необходимо назначить штатную единицу.')
        else
          PKDBDictEdit2.SetValue(ADQuery3STRUCTUREID.asstring);
     end;
    end;        }
